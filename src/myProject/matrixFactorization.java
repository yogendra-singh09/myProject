...
public class matrixFactorization {
	...
	public static void matrix_factorization(Matrix  R, Matrix P,Matrix Q1, int K){
		...
		int N=R.getRowDimension(),M=R.getColumnDimension();
		for(int step=0;step<steps;step++){
			for(int i=0;i<N;i++){
				for(int j=0;j<M;j++){
					if (R.get(i,j) > 0) // j has rated i
					{
						double eij = R.get(i,j)-getEIJ(P.getArrayCopy()[i],Q.transpose().getArray()[j]);
						for (int k=0;k<2;k++){
		...
		System.out.println("P is matrix");
		for(double[]i :P.getArray()){
			System.out.println(Arrays.toString(i));
		}

		System.out.println("Q is matrix");
		for(double[]i :Q.transpose().getArray()){
			System.out.println(Arrays.toString(i));
		}
